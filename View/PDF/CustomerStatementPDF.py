...
def genarateCustomerStatement(statementid, eodDate, errorcount, successcount):
        try:
            ...
            if df1.size != 0:
                ...
                for ind in df2.index:
                    default_value = 0
                    df2['transactionamount'] = pd.to_numeric(df2['transactionamount'], errors='coerce').fillna(default_value)

                    converted_value = int(df2['transactionamount'][ind])

                    if converted_value is not None and converted_value > 0:

                            timestamp = df2['settlementdate'][ind]

                            # Format the date as "MM/DD"
                            formatted_settlement_date = timestamp.strftime("%m/%d")

                            print(formatted_settlement_date)

                            # row 1
                            detail_row1 = shapes.Drawing(row_width, 15)
                            string_billing_date = shapes.String(0, 0, str(formatted_settlement_date), fontName="Helvetica",
                                                                fontSize=8)
                            detail_row1.add(string_billing_date)

                            timestamp = df2['transactiondate'][ind]

                            # Format the date as "MM/DD"
                            formatted_txn_date = timestamp.strftime("%m/%d")

                            print(formatted_txn_date)

                            string_txn_date = shapes.String(1 * inch, 0, str(formatted_txn_date),
                                                            fontName="Helvetica",
                                                            fontSize=8)
                            ...
                            if df2['transactionamount'][ind] is None:
                                ...
                                txn_amount += df2['transactionamount'][ind]
                ...
                if df2['cashadvancefee'][ind] is not None and int(df2['cashadvancefee'][ind]) > 0:
                    ...
                    # Format the date as "MM/DD"
                    settle_date = timestamp.strftime("%m/%d")

                    print(" txn " + settle_date)

                    string_billing_date = shapes.String(0, 0, str(settle_date), fontName="Helvetica",
                                                        fontSize=8)
                    ...
                    # Format the date as "MM/DD"
                    txn_date = timestamp.strftime("%m/%d")
                ...
                sub_report_two(df3, row_width, elements, str(df1["statementenddate"].values[0])[:10], df2)
                ...
                Dao.UpdateEodCardFeeTableBillingDone(str(df1["accountno"].values[0]), str(df1["statementenddate"].values[0])[:10])
                Dao.UpdateAdjustmentTableBillingDone(str(df1["accountno"].values[0]), format(str(statementid)), str(df1["statementenddate"].values[0])[:10])
                Dao.InsertIntoDownloadTable(str(df1["cardno"].values[0]), format(str(statementid)), str(df1["accountno"].values[0]))
        except Exception as err:
            app.logger.error(
    ...
    try:
        ...
        for ind in df3.index:
            ...
            if df3['feeamount'][ind] is not None:
                ...
                timestamp = df3['effectdate'][ind]

                # Format the date as "MM/DD"
                formatted_effect_date = timestamp.strftime("%m/%d")

                print(" Effected Date " + formatted_effect_date)

                string_billing_date = shapes.String(0, 0, str(formatted_effect_date), fontName="Helvetica",
                                                    fontSize=8)
                column_row.add(string_billing_date)

                string_txn_date = shapes.String(1 * inch, 0, str(formatted_effect_date), fontName="Helvetica",
                                                fontSize=8)
                column_row.add(string_txn_date)
        ...
        if df3['interrest'][ind] is not None and int(df3['interrest'][ind]) > 0:
            ...
            column_row = shapes.Drawing(row_width, 15)
            string_billing_date = shapes.String(0, 0, end_date, fontName="Helvetica", fontSize=8)
            column_row.add(string_billing_date)

            string_txn_date = shapes.String(1 * inch, 0, end_date, fontName="Helvetica", fontSize=8)
            column_row.add(string_txn_date)
...
def sub_report_one(df4, elements, row_width, df2):
    try:
        ...
        for ind in df4.index:
            ...
            timestamp = df4['adjustdate'][ind]

            # Format the date as "MM/DD"
            formatted_adj_date = timestamp.strftime("%m/%d")

            print(" Adjust Date " + formatted_adj_date)

            string_billing_date = shapes.String(0, 0, str(formatted_adj_date), fontName="Helvetica",
                                                fontSize=8)
            column_row.add(string_billing_date)

            string_txn_date = shapes.String(1 * inch, 0, str(formatted_adj_date), fontName="Helvetica",
                                            fontSize=8)
            column_row.add(string_txn_date)
